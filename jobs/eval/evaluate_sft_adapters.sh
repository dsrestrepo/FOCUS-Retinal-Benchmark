#!/bin/bash
#SBATCH --job-name=eval_sft_medgemma
...
#SBATCH --output=logs/eval_sft.out
#SBATCH --error=logs/eval_sft.err
...
TRAIN_STRATEGIES=$(python -c 'import yaml; print(" ".join(yaml.safe_load(open("config/mllm_config.yaml"))["sft_strategies"]))')
...
EVAL_ROOT="${OUTPUT_DIR%/*}/evals_sft"
        ...
        for train_task in $TRAIN_TASKS; do
            for train_strategy in $TRAIN_STRATEGIES; do
                ...
                if [ "$USE_UNSLOTH" = "True" ]; then
                    ADAPTER_NAME="unsloth_sft_${model_slug}_${train_dataset}_${train_task}_${train_strategy}"
                else
                    ADAPTER_NAME="sft_${model_slug}_${train_dataset}_${train_task}_${train_strategy}"
                fi
                ...
                for eval_dataset in $EVAL_DATASETS; do
                    for eval_strategy in $EVAL_STRATEGIES; do
                        echo "----------------------------------------------------------------"
                        echo "Running SFT Eval: $model + adapter trained on $train_dataset/$train_task ($train_strategy) | Eval: $eval_dataset/$eval_task | Eval Strategy: $eval_strategy"
