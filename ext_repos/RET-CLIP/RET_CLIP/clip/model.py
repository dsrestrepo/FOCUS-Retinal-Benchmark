...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor):
        ...
        weight = self.weight.type(torch.float32) if self.weight is not None else None
        bias = self.bias.type(torch.float32) if self.bias is not None else None
        ret = torch.nn.functional.layer_norm(x, self.normalized_shape, weight, bias, self.eps)
