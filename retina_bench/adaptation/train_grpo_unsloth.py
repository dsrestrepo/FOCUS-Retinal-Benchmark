...
def main():
    ...
    logger.info(f"Loading processor and model via UNSLOTH: {args.model_id}")
    ...
    # FIX: Unsloth's compiled GRPOTrainer incorrectly accesses `model.config` directly
    # instead of unwrapping the DistributedDataParallel module under torchrun SFT/GRPO.
    from torch.nn.parallel import DistributedDataParallel
    if not hasattr(DistributedDataParallel, "config"):
        DistributedDataParallel.config = property(lambda getattr_self: getattr_self.module.config)
    if not hasattr(DistributedDataParallel, "generation_config"):
        DistributedDataParallel.generation_config = property(lambda getattr_self: getattr_self.module.generation_config)
